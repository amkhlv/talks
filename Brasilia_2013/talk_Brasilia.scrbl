...
@(define bystro-conf 
   (bystro (find-executable-path "amkhlv-java-formula.sh")
           "talk_Brasilia_formulas.sqlite"  ; name for the database
           "talk_Brasilia" ; directory where to store .png files of formulas
           25  ; formula size
...
@title{Lie-algebraic interpretation of  integrated vertex operators}
Andrei Mikhailov, IFT UNESP. Talk based on: A.M., @hyperlink["http://arxiv.org/abs/1207.2441"]{http://arxiv.org/abs/1207.2441},
@hyperlink["http://arxiv.org/abs/1303.2090"]{http://arxiv.org/abs/1303.2090}, and Osvaldo Chandia, A.M., Brenno C. Vallilo,
...
@spn[attn]{vertex operators}:
@centered{
@image[#:scale 0.3]{graphics/genusThreeVert.png}
}
It is essential, that there are two types of vertex operators: @spn[attn]{integrated} and @spn[attn]{unintegrated}.
When we talk about the Hilbert space of the theory (= BRST cohomology), it is usually described in terms of the 
@spn[attn]{un}integrated vertices. Integrated vertices, on the other hand, describe the deformations of the
string @f{\sigma}-model.

In the calculation of the scattering amplitudes, we  need to know both types of operators.

They form the same space of states, there is a one-to-one correspondence  between them: the @spn[attn]{descent procedure}.
We will give some algebraic interpretation of this descent procedure, which  hopefully makes it more transparent.

We will start with a very brief review of the @spn[attn]{pure spinor superstring}.

}



@slide["Pure spinor superstring" #:tag "PureSpinorSuperstring" #:showtitle #t]{
...
@slide["Unintegrated vertices" #:tag "ReviewUnintegrated" #:showtitle #t]{
...
@spn[attn]{Unintegrated vertex} is an operator @f{V(x,\theta,\lambda)} of the conformal dimension zero 
and ghost number two, annihilated by @f{Q}.
...
It is a fundamental principle of the string theory, that @spn[attn]{every state corresponds to some deformation of
the worldsheet theory}. This leads to the concept of @bold{integrated vertex}, which we will now describe.
}


@slide["Integrated vertices" #:tag "ReviewIntegrated" #:showtitle #t]{
...
@slide["Simplest example of the pure spinor formalism" #:tag "SimplestExample" #:showtitle #t]{
...
We will now present some algebraic point of view on this BRST complex.
}


@slide["Algebra formed by covariant derivatives" #:tag "AlgebrasOfCovariantDerivatives" #:showtitle #t]{
...
@slide["BRST cohomology vs Lie algebra cohomology" #:tag "BRSTVsLie" #:showtitle #t]{
@table-of-contents[]
@section[#:tag "RelationBetweenSYMAndSUSY" ]{Relation between the SYM algebra and the susy algebra}
...
@section{Cohomology of SYM algebra and pure spinor cohomology}
...
@div[comment]{The action of @f{Q} on @f{p(\lambda)\otimes v} where @f{p} is a polynomial of degree @f{n} of @f{\lambda} and @f{v\in V\,}
is equal to @f{p(\lambda)\lambda^{\alpha}\otimes S_{\alpha}v}. Notice that @f{p(\lambda)\lambda^{\alpha}} is a polynomial
of the degree @f{n+1} in @f{\lambda}, so we got an element of @f{{\cal P\,}^{n+1}\otimes V \longrightarrow \ldots}.
}
...
@div[comment]{
An important special case is when @f{V\;} is 
...
In this case the operator (@ref{BRSTForSYMvsLie}) becomes precisely (@ref{SimpleQ}).
}
}


@slide["What about SUGRA?" #:tag "TowardsSUGRA" #:showtitle #t]{
...
In this case, we can define the BRST cohomology based on the @bold{representations of the superconformal algebra}.
}

@slide["Superconformal algebra" #:tag "SuperconformalAlgebra" #:showtitle #t]{
@table-of-contents[]
@section{General definition and generators}
...
@section{Structure of commutation relations}
...
@slide[@elem{BRST cohomology of Type IIB in @f{AdS_5\times S^5}} #:tag "AdSBRST" #:showtitle #t]{
We want to construct some analogue of (@ref{PureSpinorBRSTComplexWithV}), (@ref{BRSTOperatorOfSYM}) where the algebra @f{\rm susy}
would be replaced with the superconformal algebra @f{{\bf g} = {\bf psu}(2,2|4)}.

The simplest naive guess: given any representation @f{V} of @f{{\bf g}}, consider the same complex as in 
(@ref{PureSpinorBRSTComplexWithV}):
@equation{
\ldots\longrightarrow {\cal P\,}^n\otimes V \xrightarrow{Q} {\cal P\,}^{n+1}\otimes V \longrightarrow \ldots
}
where now @f{{\cal P\,}^n} is the space of @f{n}-th degree polynomials of @bold{two} pure spinors @f{\lambda_L} and @f{\lambda_R}
and the operator @f{Q} is defined just as a sum of two expressions like (@ref{BRSTOperatorOfSYM}):
@equation{
Q = \lambda_L^{\alpha}\, t^3_{\alpha} + \lambda_R^{\dot{\alpha}}\,t^1_{\dot{\alpha}}
}
This is @italic{almost} correct, except for such an operator would @bold{not} be nilpotent:
@equation{
Q^2 = \lambda_L^{\alpha}\lambda_R^{\dot{\beta}} \; f_{\alpha\dot{\beta}}{}^{\cal A}\; t^0_{\cal A} \neq 0
}
The way out is to impose some condition of @f{{\bf g}_{\bar{0}}}-invariance, either this:
@equation[#:label "G0CoInvariants"]{
\ldots\longrightarrow {\cal P\,}^n\otimes_{{\bf g}_{\bar{0}}} V \xrightarrow{Q} {\cal P\,}^{n+1}\otimes_{{\bf g}_{\bar{0}}} V \longrightarrow \ldots
}
or maybe this:
@equation[#:label "G0Invariants"]{
\ldots\longrightarrow \left({\cal P\,}^n\otimes V\,\right)^{{\bf g}_{\bar{0}}} \xrightarrow{Q} 
\left({\cal P\,}^{n+1}\otimes V\,\right)^{{\bf g}_{\bar{0}}} \longrightarrow \ldots
}
@div[comment]{
There is a subtle difference between these two definitions.
Here @f{\left({\cal P\,}^n\otimes V\,\right)^{{\bf g}_{\bar{0}}}} is the space of @f{{\bf g}_{\bar{0}}}-invariants,
while @f{{\cal P\,}^n\otimes_{{\bf g}_{\bar{0}}} V} are ``co-invariants''. When @f{V\,} is finite-dimensional, they are the same. 
But we often need infinite-dimensional @f{V\,} such as @f{V = (U{\bf g})'}. In this case, we typically need (@ref{G0Invariants}).
}
Imposing such an invariance condition makes @f{Q} nilpotent. For example, in case of (@ref{G0CoInvariants}):
@align[r.c.l @list[
@f{p(\lambda_L,\lambda_R)\;\;}
@f{\otimes_{{\bf g}_{\bar{0}}}\;\;}
@v-[4 @f{\lambda_L^{\alpha}\lambda_R^{\dot{\beta}} \; f_{\alpha\dot{\beta}}{}^{\cal A}\; t^0_{\cal A}\;v \;=\;}]
]@list[
@f{\;=\;
\lambda_L^{\alpha}\lambda_R^{\dot{\beta}} \; f_{\alpha\dot{\beta}}{}^{\cal A}
\left(
f_{{\cal A}\gamma}{}^{\delta}\lambda^{\gamma}{\partial\over\partial \lambda^{\delta}}\;+\;
f_{{\cal A}\hat{\gamma}}{}^{\hat{\delta}}\lambda^{\hat{\gamma}}{\partial\over\partial \lambda^{\hat{\delta}}}
\right)p(\lambda_L,\lambda_R)\;\;
}
@v+[4 @f{\otimes_{{\bf g}_{\bar{0}}}\;\;}] @v+[6 @f{v\;\;=\;\;0}]]]
This is zero because of the Jacobi identity and pure spinor constraints.

Eq. (@ref{G0Invariants}) is the correct definition of the BRST complex, and the one which actually comes from
the pure spinor formalism.
}

@slide[@elem{Algebra @f{{\cal L}_{\rm tot}}} #:tag "AlgebraLtot" #:showtitle #t]{

We will now extend the superconformal algebra to some infinite-dimensional algebra, just like @f{\rm susy} 
@seclink["RelationBetweenSYMAndSUSY"]{was extended} to SYM.
...
The @bold{consistency} of this definition can be verified using the general theory of quadratic-linear
algebras, the @bold{PBW theorem} by Polishchuk-Positselski-Braverman-Gaitsgory. 

@div[comment]{Since the associated graded algebra is Koszul, it is enough to verify
the Jacobi identity for three elementary generators. But for any three elementary generators, the commutation
relations are the same as in the twisted loop algebra @f{L{\bf psu}(2,2|4)}.}

@section[#:tag "SectionBRSTVsLie"]{BRST cohomology as a relative cohomology}
...
Therefore an element of @f{C^n} is a function of @f{n} Lie-algebra valued variables with taking values in @f{V}:
@equation{
\psi(\xi_1,\ldots,\xi_n)\in V
}
The actual map between @f{H^n({\cal L}_{\rm tot}\;,\;{\bf g}_{\bar{0}}\;,\;V\,)} and @f{H^n(Q,V\,)}
goes as follows. Given a representative cocycle @f-3{\psi\in\mbox{Hom}_{{\bf g}_{\bar{0}}}\left( 
\Lambda^n\left( {{\cal L}_{\rm tot}\over {\bf g}_{\bar{0}}} \right)\;,\;V\right)}, 
the representative of the corresponding element in @f{H^n(Q,V\,)} is obtained by the substitution of 
@f-3{\lambda_L^{\alpha}\nabla^L_{\alpha} + \lambda_R^{\hat{\alpha}}\nabla^R_{\hat{\alpha}}} in place of
@f{\xi_1,\ldots,\xi_n}:
@equation[#:label "ActualMap"]{
\psi \mapsto \psi\left(\;
\lambda_L^{\alpha}\nabla^L_{\alpha} + \lambda_R^{\hat{\alpha}}\nabla^R_{\hat{\alpha}}\;\;,\;\;\ldots\;\;,\;\;
\lambda_L^{\alpha}\nabla^L_{\alpha} + \lambda_R^{\hat{\alpha}}\nabla^R_{\hat{\alpha}}
\;\right)
}
...
@slide["My Ideal" #:tag "MyIdeal" #:showtitle #t]{
@larger{Algebraic interpretation of SUGRA operators}

There is a relation analogous to (@ref{RelationBetweenSYMAndSUSY}), namely exists an ideal @f{I\subset {\cal L}_{\rm tot}}
such that:
@equation{
{\bf g} = {\bf psu}(2,2|4) = {\cal L}_{\rm tot}/I
}
...
@slide["Lax operator" #:tag "Lax" #:showtitle #t]{
...
For the pure spinor superstring in @f{AdS_5\times S^5} the Lax pair was constructed by 
...
@f{J} and @f{N} are @bold{currents}: 
@equation[#:label "Jvsg"]{
   J_{\bar{k}} = -(dg g^{-1})_{\bar{k}}
}
@equation{
N_+ = \{w_+,\lambda_L\}\;,\;\; N_- = \{w_-,\lambda_R\}
}
...
@slide["Generalizations of the Lax pair" #:tag "GeneralizedLax" #:showtitle #t]{

@table-of-contents[]

@section{Lax operator and the loop algebra}
...
@slide["Properties of the generalized Lax pair" #:tag "PropertiesOfGeneralizedLax" #:showtitle #t]{
...
@slide["Integrated Vertex in AdS" #:tag "ConstructionOfIntegratedVertex" #:showtitle #t]{

Finally, we are now ready to discuss our construction of the integrated vertex.

@table-of-contents[]
@section{Construction of the integrated vertex}
...
@section{Proof of the descent procedure}
We have to verify the descent procedure (@ref{DefU}), (@ref{DefW}).
...
with @f-3{\epsilon(\lambda_L^{\alpha} \nabla^L_{\alpha} + \lambda_R^{\hat{\alpha}}\nabla^R_{\hat{\alpha}})} --- cp. Eq. (@ref{ActualMap}).
...
fields with @f{\lambda_L + \lambda_R}, @seclink["SectionBRSTVsLie"]{as it should be}.
}


@slide["Instead of conclusion" #:tag "InsteadOfConclusion" #:showtitle #t]{
